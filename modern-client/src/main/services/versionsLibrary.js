...
function listInstalled({ withSize = false } = {}) {
...
function isSnapshotVersion(mcVersion) {
  if (!mcVersion) return false;
  return !/^\d+\.\d+(\.\d+)?$/.test(mcVersion);
}

module.exports = {
  recordMeta, recordMetaIfMissing, getMeta, listInstalled, deleteInstalled, isSnapshotVersion,
};
