...
function isModernNativesClassifier(name) {
  const parts = String(name || '').split(':');
  return parts.length >= 4 && /^natives-/.test(parts[3]);
}

function nativesClassifierMatchesArch(classifier, platform) {
  const match = /^natives-(?:windows|linux|macos)(?:-(x86|arm64))?$/.exec(classifier);
  if (!match) return true;
  const arch = match[1];
  if (!arch) return platform.arch === 'x64';
...
function resolveLibraries(versionJson, platform = getPlatform()) {
  ...
  for (const lib of versionJson.libraries || []) {
    ...
    if (artifact) {
      const modernNatives = isModernNativesClassifier(lib.name);
      const wrongArch = modernNatives && !nativesClassifierMatchesArch(lib.name.split(':')[3], platform);
      if (modernNatives && !wrongArch) {
        natives.push({
          name: lib.name,
