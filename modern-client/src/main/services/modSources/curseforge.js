...
function normalizeFile(modId, file, modSlug) {
  ...
  return {
    ...
    webUrl: modSlug ? `https://www.curseforge.com/minecraft/mc-mods/${modSlug}` : null,
...
async function search({
  query = '', mcVersion, loader, index = 0, pageSize = 20,
} = {}) {
  const params = new URLSearchParams({
    gameId: String(GAME_ID), classId: String(CLASS_ID_MODS), searchFilter: query, index: String(index), pageSize: String(pageSize),
  });
...
async function resolveInstallFile(modId, { versionId, mcVersion, loader } = {}) {
  ...
  if (versionId) {
    ...
    const data = await res.json();
    return normalizeFile(modId, data.data, slug);
  }
  const files = await getFiles(modId, { mcVersion, loader });
  if (!files.length) throw new Error('호환되는 CurseForge 파일을 찾을 수 없습니다.');
  return normalizeFile(modId, files[0], slug);
}

module.exports = {
  search, getMod, getFiles, resolveInstallFile,
};
