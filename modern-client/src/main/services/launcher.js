...
const mcSettings = require('./mcSettings');
...
async function runLaunch(launchId, instanceId, handlers, { force = false } = {}) {
  ...
  const isSnapshot = versionMeta.loader !== 'vanilla' && versionsLibrary.isSnapshotVersion(versionMeta.mcVersion);
  if (isSnapshot && !force) {
    emit(handlers, 'onSnapshotWarning', { launchId, mcVersion: versionMeta.mcVersion });
    return;
  }
  ...
  mcSettings.applyToOptionsFile(instanceId);
  ...
  const javaOptions = overrides.javaOptions || settings.javaOptions;
  ...
  const { argv } = argsBuilder.buildLaunchArgs({
    versionJson, session, gameDir, assetsRoot, nativesDir, classpathEntries, javaOptions,
  });

  let child;
  try {
    child = spawn(javaPath, argv, { cwd: gameDir });
  } catch (err) {
  ...
  child.stdout.on('data', (chunk) => {
    ...
    pushTail(text);
    emit(handlers, 'onLog', { launchId, stream: 'stdout', line: text });
  });
  child.stderr.on('data', (chunk) => {
    const text = chunk.toString();
    pushTail(text);
    emit(handlers, 'onLog', { launchId, stream: 'stderr', line: text });
  });
  ...
  child.on('close', (code, signal) => {
    activeProcesses.delete(launchId);
    const wasCancelled = cancelledLaunches.delete(launchId);
    settingsSvc.addRecentLaunch({
      instanceId, instanceName: instance.name, versionId: instance.versionId, timestamp: Date.now(),
    });
    const crashed = !wasCancelled && code !== 0 && code !== null;
    if (crashed) {
      emit(handlers, 'onCrash', {
        launchId, code, signal, logTail: logTail.slice(-30),
      });
    }
    emit(handlers, 'onExit', { launchId, code, signal, crashed });
  });
}
